...
class IP_summarizer:
    ...
    def do_PCA(self):
        ...
        for k in range(1):
            self.Doc_mat_ = attention(self.Doc_mat_, self.Doc_mat_, self.Doc_mat_)
    ...
    def sent_importance(self, Doc_mat):
        ...
        normalize_factor = np.matmul(np.reshape(sq_D_sum, [-1, 1]), np.transpose(np.reshape(sq_P_sum, [-1, 1])))

        unnormalized_similarity = np.matmul(Doc_mat, self.PCs)
        normalized_similarity = np.multiply(normalize_factor, unnormalized_similarity)
        weighted_similarity = (self.var_ratio[:self.N]/100) * np.exp(normalized_similarity)
    ...
    def sent_importance_loading(self):
        ...
        importance = []
        v_ref = np.array([1 / np.sqrt(len(self.var_ratio))] * len(self.var_ratio))
        for i in range(len(self.var_ratio)):
            imp = np.sum(self.var_ratio * np.maximum((np.abs(self.alpha_M[:, i]) - v_ref), 0))
    ...
    def pairwise_sent_similarity(self,Doc_mat):
        ...
        normalize_factor = np.matmul(np.reshape(sq_sum, [-1, 1]), np.transpose(np.reshape(sq_sum, [-1, 1])))
    ...
    def Optimization(self):
        ...
        ys = ['sent_%dsent_%d' % (i + 1, j + 1) for i in range(len(self.imp_pr)) for j in range(i + 1, len(self.imp_pr))]

        sentence_importance = dict((s, self.imp_pr[i]) for i, s in enumerate(sentences))
